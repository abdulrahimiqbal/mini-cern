...
"""
    ...
    async def _call_model(self, 
        ...
        provider = self.providers['openai']  # Default to OpenAI for now
    ...
    async def analyze_literature(self, 
                               topic: str, 
                               papers: List[Dict[str, Any]]) -> LLMResponse:
        """Analyze and synthesize literature on a topic"""
        
        context = ResearchContext(
            domain="literature_analysis",
            topic=topic,
            previous_findings=[],
            constraints={"paper_count": len(papers)},
            priority="medium",
            budget_remaining=20000.0
        )
        
        papers_summary = "\n".join([
            f"- {paper.get('title', 'Unknown')}: {paper.get('abstract', 'No abstract')[:200]}..."
            for paper in papers[:10]  # Limit to first 10 papers
        ])
        
        prompt = f"""
        Analyze the following literature on "{topic}":
        ...
        6. Quality assessment of the body of work
        
        Synthesize into a coherent analysis that could guide future research.
        """
        
        return await self.route_task(TaskType.LITERATURE, prompt, context)
    
    async def validate_safety(self, 
                             research_proposal: str, 
                             domain: str) -> LLMResponse:
        """Validate safety and ethical aspects of research"""
        
        context = ResearchContext(
            domain=domain,
            topic="safety_validation",
            previous_findings=[],
            constraints={},
            priority="critical",
        ...
        prompt = f"""
        Evaluate the safety and ethical implications of this research proposal:
        
        "{research_proposal}"
        
        Assess:
        1. Potential physical/environmental risks
        2. Ethical considerations
        3. Dual-use concerns
        4. Data privacy and security
        5. Regulatory compliance requirements
        6. Recommended safety measures
        7. Risk mitigation strategies
        8. Monitoring and oversight needs
        
        Provide a comprehensive safety assessment with recommendations.
        """
        
        return await self.route_task(TaskType.SAFETY, prompt, context)
